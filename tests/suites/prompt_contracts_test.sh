...
test_runthrough_qa_checklist_prompt_defines_execution_and_progress_contract() {
  ...
  assert_contains "$prompt" "comments or product code"
...
test_multi_axis_pr_review_prompt_defines_four_skill_vote_contract() {
  ...
  assert_contains "$prompt" "Spawn exactly one top-level subagent"
  assert_contains "$prompt" "complete both its Standards and"
  assert_contains "$prompt" "Spec passes itself"
  assert_contains "$prompt" "must not spawn further subagents"
  assert_contains "$prompt" "Wait for the wave 1 result"
  assert_contains "$prompt" "wave 2 in parallel"
  assert_contains "$prompt" "Spawn exactly three subagents concurrently"
  assert_contains "$prompt" "return findings only to the parent"
  ...
  assert_contains "$prompt" "four top-level review results"
