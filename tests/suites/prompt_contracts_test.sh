...
test_multi_axis_pr_review_prompt_defines_four_skill_vote_contract() {
  ...
  assert_contains "$prompt" "Spawn exactly one subagent"
  assert_contains "$prompt" "Require both nested results"
  ...
  assert_contains "$prompt" "replace missing delegated work with its own review"
