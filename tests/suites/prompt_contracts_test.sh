...
test_multi_axis_pr_review_prompt_defines_four_skill_vote_contract() {
  ...
  assert_contains "$prompt" "two independent delegated results"
  assert_contains "$prompt" "Standards result"
  assert_contains "$prompt" "Spec result"
  assert_contains "$prompt" "Wait for both wave 1 results"
  ...
  assert_contains "$prompt" "Do not begin Consolidate"
  assert_contains "$prompt" "five delegated results"
