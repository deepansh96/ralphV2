...
Review the full change from the perspective of a senior engineer, along two separate axes. SPEC: does the change faithfully implement the parent issue's PRD — missing or partial requirements, scope creep, requirements implemented wrong. STANDARDS: correctness bugs, architectural concerns, security issues, missing edge cases, plus the Fowler smell baseline in {{SKILLS_DIR}}/code-review/SKILL.md — read that file and match its listed smells against the diff. Every smell is a judgement call, never a hard violation, and a documented repo standard overrides the baseline. Ignore style nitpicks and anything a linter would catch. Report the two axes separately. For each issue found, state the axis, severity (critical/major/minor), and a concrete recommendation."
```
...
After verification, synthesize the verified council findings and the selected automated review findings into a single combined review, keeping the Spec and Standards axes as separate sections rather than reranking across them. For the Claude path, use the `code-review:code-review` findings from PR comments. For the Codex path, use `{{WORKSPACE}}/codex-pr-review.md`. Filter the council findings using the same rules as other review steps: keep critical and major issues, drop nitpicks, style-only comments, and points invalidated by codebase verification. Keep smell-baseline findings that survive verification, labelled as judgement calls (e.g. "possible Feature Envy"), not as blockers. Include the automated review source (`code-review:code-review` or `codex review`) and a council attribution line at the end of the comment (e.g., `Reviewed by: codex, gemini, kimi · Failed: deepseek` or `Reviewed by: codex, gemini, kimi, deepseek` if none failed). Post the combined review as a PR comment using `gh pr comment`.
