...
**Date:** 2026-08-24
**Status:** Active
**Decision:** Step prompts may request one reusable provider-specific native delegation contract selected from their main `agent`. Codex workers use `gpt-5.6-luna` with `max` reasoning through native subagents. Claude Code uses its native dynamic Workflow tool so workers use `sonnet` with `high` effort. Each step defines its own flat topology: multi-axis review launches one Matt worker followed by three concurrent skill workers, and the Matt worker performs its Standards and Spec passes itself; QA lets the main agent form dependency-safe worker batches while keeping shared-resource mutations serialized. Ralph renders the instructions but never launches or coordinates workers itself.
**Reasoning:** Claude Code and Codex are post-trained to operate their own delegation tools, but workers inherit the parent configuration when the prompt does not name a model and effort. Codex can set both values on a subagent spawn. Claude Code's plain Agent tool can set a model per invocation but cannot set effort, while its Workflow `agent()` function can set both; live testing also confirmed that a workflow worker cannot spawn a second-level Agent. A flat topology works on both providers, keeps the main agent in control, and lets the same provider contract serve review, QA, and future delegated steps without carrying step-specific policy.
...
**Consequences:** Delegated QA and multi-axis review steps support Claude or Codex as their main agent and fail during prompt rendering for an unconfigured provider. Claude main agents explicitly opt in to a session-scoped dynamic workflow; Codex main agents use native subagent calls directly. Workers never spawn workers. Each main agent preserves its step's dependencies, owns shared writes and final verification, and must fail rather than silently falling back when its native tool cannot apply the requested worker settings.
