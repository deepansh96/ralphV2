...
- **`scripts/grill.sh`** - Automated Grilling Session coordinator behind `ralph.sh grill`, dispatched before the `--issue` parser. `start` validates inputs, checks adapter capabilities, enforces branch safety in the target repository (Git toplevel above the Ralph directory), writes the Grilling Session Record, and starts the Grilling then Answering native sessions with distinct IDs, then runs the relay loop (Frontier → Answers → merged `decisions` → next Frontier; on an empty Frontier, summary draft → one review → summary final) and writes `confirmation.md` at the `blocked/awaiting_confirmation` gate. `resume --id` continues a `grilling` session with the frozen configuration. Exchanges are sequential (`ex-NNNN`), recorded `intent` → `sent` → `completed`, and validated with jq. It is the only component that writes the record or runs Git/`gh` writes for grilling.
- **`scripts/grill-record.sh`** - Grilling Session Record store at `grilling-sessions/<id>/session.json`; jq-only atomic write-then-rename under umask 077 (dirs `0700`, files `0600`). Separate from pipeline State.
- **`scripts/grill-adapters.sh`** - session-adapter contract (`adapter_capabilities`, `adapter_new_native_id`, `adapter_start`, `adapter_send`) with the Claude implementation: coordinator-generated `--session-id`, `--resume <id>` with a `--json-schema` hint for every later exchange (never `--continue`/`--last`/`--fork-session`), `acceptEdits` in the repo for the Grilling Agent, `default` mode with write tools disallowed for the Answering Agent, and sandbox settings denying Git/`gh` writes for both.
- **`prompts/grill/`** - Grilling Agent and Answering Agent role prompts plus per-exchange fragments (session start, round, closing) carrying `[ralph-exchange:<id>]`, and message JSON Schemas in `prompts/grill/schemas/`.
