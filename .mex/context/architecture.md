...
- **`scripts/grill-adapters.sh`** - session-adapter contract (`adapter_capabilities`, `adapter_new_native_id`, `adapter_start`) with the Claude implementation: coordinator-generated `--session-id`, `acceptEdits` in the repo for the Grilling Agent, `default` mode with write tools disallowed for the Answering Agent, and sandbox settings denying Git/`gh` writes for both.
- **`prompts/grill/`** - Grilling Agent and Answering Agent role prompts plus the session-start fragment carrying `[ralph-exchange:<id>]`.
