...
- Prompt templates in `prompts/` render step-specific instructions and dispatch through `scripts/agent.sh` to Claude, Codex, or DeepSeek through Pi. Multi-axis review injects provider-native worker settings for Claude or Codex while leaving all spawning and coordination to the main agent. Init installs always-run local-resource cleanup; preflight appends implementation steps followed by final checks, PR creation, local QA preparation/execution, and four-axis PR review.
