...
- Pipeline state lives in `workspaces/<issue>/state.json` with fixed steps, dynamic steps, optional per-step main-agent and subagent model/reasoning overrides, metrics, HITL flags, and stale PID recovery. Repo-wide subagent defaults live in `ralph.config.json`.
- Prompt templates in `prompts/` render step-specific instructions and dispatch through `scripts/agent.sh` to Claude, Codex, or DeepSeek through Pi. QA execution and multi-axis review inject the same reusable provider-native worker contract for Claude or Codex, with model and effort resolved from step overrides then Ralph config, while leaving flat spawning and coordination to the main agent. Init installs always-run local-resource cleanup; preflight appends implementation steps followed by final checks, PR creation, local QA preparation/execution, and five-pass PR review.
