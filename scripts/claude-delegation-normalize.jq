...
# Lifecycle marks are positions in this attempt's append-only hook log. A
# PreToolUse hook completes before its worker starts and every end hook fires
# after the worker returned, so an end position below a later start position
# proves the two runs did not overlap.
($events | to_entries) as $log |
$pres | map(. as $pre |
  ...
  # A foreground return (terminal PostToolUse status) or SubagentStop proves the
  # worker ended; a background launch without its stop may still be running.
  (if $completed or $failed or $stopped then
     if ($post.status | IN("completed","failed","stopped")) or ($stops|length == 1) then
       [$log[] | select(((.value.event | IN("PostToolUse","PostToolUseFailure")) and .value.tool_use_id == $pre.tool_use_id)
         or (.value.event == "SubagentStop" and .value.agent_id == $post.agentId)) | .key] | max
     else null end
  ...
  {childId:$post.agentId,parentId:($pre.agent_id // $parent),taskId:$pre.taskId,run:$pre.run,
   ...
   startedAt:([$log[] | select(.value.event == "PreToolUse" and .value.tool_use_id == $pre.tool_use_id) | .key] | first),
   endedAt:$endedAt,
