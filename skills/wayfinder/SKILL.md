...
**Where the map, its child tickets, blocking, and frontier queries physically live is tracker-specific.** Resolve the tracker doc through the `Issue tracker` pointer in the root `AGENTS.md` and consult its "Wayfinding operations" section for how *this* repo expresses them. For Ralph that doc is `docs/agents/issue-tracker.md` (GitHub).
...
- **Prototype** (HITL): Raise the fidelity of the discussion by making a cheap, rough, concrete artifact to react to — an outline, a rough take, a stub, or throwaway UI/logic code. Keep prototype code out of the mainline; link it as an asset. Use when "how should it look" or "how should it behave" is the key question.
